...
def palavra(request, palavra):
    palavra_list = Palavra.objects.filter(palavra__exact = palavra).order_by('votes')
    mais_votada = palavra_list[0]
    letra = mais_votada.palavra[0]
    palavra_side_list = Palavra.objects.filter(palavra__istartswith = letra).filter(validacao__exact='ok').values('palavra').distinct().order_by('palavra')
...
def palavraid(request, palavra, palavra_id):
    ...
    letra = mais_votada.palavra[0]
    palavra_side_list = Palavra.objects.filter(palavra__istartswith = letra).values('palavra').distinct().order_by('palavra')
    return render_to_response('dicionario/palavra.html', {'palavra_side_list': palavra_side_list, 'palavra_list': palavra_list, 'mais_votada': mais_votada, 'letra': letra})
...
def search(request):
    ...
    palavra_list = Palavra.objects.filter(palavra__istartswith = crit).order_by('palavra')
    letra = crit[0]
    palavra_side_list = Palavra.objects.filter(palavra__istartswith = letra).filter(validacao__exact='ok').values('palavra').distinct().order_by('palavra')    
